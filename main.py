"""
Portfolio Diversification – Strategic & Tactical Asset Allocation
Gabungan dua model dalam satu aplikasi Streamlit
...
def parse_tactical_inputs(assets_str, prices_str):
...
st.markdown("**Mean-Variance Optimization (Markowitz / SLSQP)** — AIMMS Chapter 18")

# ── PILIHAN MODE ──
mode = st.radio(
    "Pilih Model Optimasi:",
    options=["🏛️ Strategic Asset Allocation", "⚡ Tactical Asset Allocation"],
    horizontal=True,
)
st.divider()

is_strategic = mode.startswith("🏛️")
...
93.6,92.7,80.5,164.0,139.3"""
    ...
    if apply_btn_t or st.session_state.t_assets is None:
        try:
            assets, prices = parse_tactical_inputs(assets_input, prices_input)
            n_a = len(assets)
            if prices.shape[1] != n_a:
                st.session_state.t_error = (
                    f"❌ Kolom harga ({prices.shape[1]}) ≠ jumlah aset ({n_a})."
                )
            elif prices.shape[0] < 2:
                st.session_state.t_error = "❌ Data harga minimal 2 baris (2 periode)."
            else:
